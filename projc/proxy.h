...
class CProxy{
	...
	int dead_router[MAX_ROUTER_COUNT];
	int array_of_paths[20][MAX_ROUTER_COUNT];
	unsigned long packet_count[MAX_CIRCUITS];
	unsigned char aes_key[KEY_LEN];
	unsigned long _old_src;
	unsigned short _old_port;
        unsigned int _source_circuit_array[100];
	unsigned int _dest_circuit_array[100];
	...
	int construct_kill_router_msg(char* buf, int len, unsigned short cID);
